...
This objective function optimizes towards several molecules at the same time, and is part of the **median molecules** family of GuacaMol {cite:p}`Brown:guacamol:2019` benchmark. We compute it using [the Therapeutics Data Common's oracle](https://tdcommons.ai/functions/oracles/#rediscovery) {cite:p}`Huang:TDC:2021`.
...
# Creating a problem
problem = Median1ProblemFactory().create()
...
# Querying:
y = f(x)
print(y)  # Should be close to [[0.09722244]]
```

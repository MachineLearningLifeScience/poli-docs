...
This objective function optimizes towards Troglitazone, and is part of the **rediscovery** family of GuacaMol {cite:p}`Brown:guacamol:2019` benchmark. We compute it using [the Therapeutics Data Common's oracle](https://tdcommons.ai/functions/oracles/#rediscovery) {cite:p}`Huang:TDC:2021`.
...
# Creating a problem
problem = TroglitazoneRediscoveryProblemFactory().create()
...
# Querying:
y = f(x)
print(y)  # Should be close to [[0.24427481]]
```

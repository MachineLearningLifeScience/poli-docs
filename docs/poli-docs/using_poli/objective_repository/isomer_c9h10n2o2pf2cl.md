...
This objective function optimizes towards a given SMILES/SELFIES that has a molecular formula of C9H10N2O2PF2Cl, and is part of the **Isomers** family of GuacaMol {cite:p}`Brown:guacamol:2019` benchmark. We compute it using [the Therapeutics Data Common's oracle](https://tdcommons.ai/functions/oracles/#multi-property-objective-mpo) {cite:p}`Huang:TDC:2021`.

:::{warning}

There is currently a discrepancy between what TDC reports in their documentation, and the values we get when we run this oracle. [Read more here](https://github.com/mims-harvard/TDC/issues/244).
...
# Creating a problem
problem = IsomerC9H10N2O2PF2ClProblemFactory().create()
...
# Querying:
y = f(x)
print(y)  # Should be close to [[1.71390843e-15]]
```
